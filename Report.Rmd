...
The above chunk of code gives a partition of the dataset for edx and validation our dataset. It also removes the unnecessary files from the working directory, which is always a good coding practice ('always wash your hands after you touch objects'). The datasets were downloaded and saved as .rds file.
...
rmses <- sapply(lambdas, function(l){
  ...
  return(RMSE(test_set_df$rating,predicted_ratings))
})
```
...
```
```{r checking the model, echo=FALSE}
lambda <- 4.75
# For t lambda,find b_i & b_u, followed by rating prediction & testing
