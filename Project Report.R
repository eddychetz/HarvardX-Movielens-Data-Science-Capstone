...
Let's load some important library packages we might need during the process:
            ...
            ```{r variables data types, echo=FALSE}
            #checking on the data types of the dataset columns
            str(edx)
            ```
            
            ```{r dimensions of edx set, echo=FALSE}
            ...
            ```{r summarizing edx dataset, echo=FALSE}
...
```{r checking variability of ratings, echo=FALSE}
...
```{r qplot for title avaerages, echo=FALSE}
...
```{r Reporting the final RMSE, echo=TRUE}
#calculate just the average of ratings
